    ...
    m_metaInfoFontSize(12)
{
    ...
    m_linkUpdateMgr = new LinkUpdateManager;
    m_nodeUpdateMgr = new NodeUpdateManager;
...
AnimLink *
AnimatorScene::getAnimLink(uint32_t fromId, uint32_t toId)
{
    AnimLink * theLink = 0;
    for(QVector <AnimLink *>::const_iterator i = m_pointToPointLinks.begin();
                       ...
                       QString linkDescription)
{
    ...
    m_animNodes[fromId]->setPos(line.p1());
    m_animNodes[toId]->setPos(line.p2());
    AnimLink * item = new AnimLink(fromId, toId, line, fromNodeDescription, toNodeDescription, linkDescription);

    m_pointToPointLinks.push_back(item);
    addItem(item);
    m_linkUpdateMgr->addLinkUpdate(0, item, linkDescription);
...
{
    AnimLink * link = getAnimLink(fromId, toId);
    if(link)
    {
        m_linkUpdateMgr->addLinkUpdate(updateTime, link, linkDescription);
    }
}

void
AnimatorScene::updateNode(qreal updateTime, uint32_t nodeId, uint8_t r, uint8_t g, uint8_t b, QString description, bool visible)
{
    AnimNode * animNode = getAnimNode(nodeId);
    if(animNode)
    {
        m_nodeUpdateMgr->addNodeUpdate(updateTime, animNode, r, g, b, description, visible);
    }
}
                       ...
                       QColor * color)
{
    ...
    if (color)
        m_nodeUpdateMgr->addNodeUpdate(0, aNode, color->red(), color->green(), color->blue(), description, true);
    else
        m_nodeUpdateMgr->addNodeUpdate(0, aNode, 255, 0, 0, description, true);
...
AnimNode *
AnimatorScene::getAnimNode(uint32_t nodeId)
{
    AnimNode * aNode = 0;
    if (m_animNodes.find(nodeId) != m_animNodes.end())
    {
...
AnimatorScene::timeToUpdate(double currentTime, bool fastFwd)
{
    ...
    if (m_linkUpdateMgr->updateLinks(currentTime))
        masterUpdate = true;
    if(m_nodeUpdateMgr->updateNodes(currentTime))
        masterUpdate = true;
...
AnimatorScene::systemReset()
{
  ...
  //  qDebug(QString("All animPackets cleared"));
    ...
    m_linkUpdateMgr->systemReset();
    delete m_linkUpdateMgr;
    m_linkUpdateMgr = new LinkUpdateManager;

    m_nodeUpdateMgr->systemReset();
    delete m_nodeUpdateMgr;
    m_nodeUpdateMgr = new NodeUpdateManager;
