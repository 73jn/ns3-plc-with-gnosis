...
bool node_t::handle_global(string func, list<string> *locals,
    ...
    // Another special case. This time, using "typeof()" to get at a function
    // type. Consider:
    //
    //   int foo() {}
    //   extern typeof(foo) foo;
    //
    // We're not smart enough to handle typeof() fully, but the exact case
    // above is at least handled by the following code.
    node_t *typeof_node = declaration_specifiers->get_typeof();
    if (typeof_node) {
      // typeof ( expr )
      
      list<node_t*>::iterator i = typeof_node->children.begin();
      ++i; ++i;
      node_t *typeof_expr = *i;

      if (!typeof_expr->s_identifier.empty()) {
        if (is_function(typeof_expr->s_identifier)) {
          is_function_type = true;
        }
      }
    }
